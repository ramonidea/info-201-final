...
sea.music.url <-
  paste0(
    ...
    "events.json?apikey=",
    api.key,
    '&size=200&city=Seattle%20&countryCode=US&classificationName=Music'
  )
sea.response <- GET(sea.music.url)
sea.body.data <-
  fromJSON(
    content(sea.response, "text"),
    simplifyVector = TRUE,
...
la.music.url <-
  paste0(
    ...
    "events.json?apikey=",
    api.key,
    '&size=200&city=Los%20Angeles&countryCode=US&classificationName=Music'
  )
la.response <- GET(la.music.url)
la.body.data <-
  fromJSON(
    content(la.response, "text"),
    simplifyVector = TRUE,
    simplifyDataFrame = TRUE
  )
...
ny.music.url <-
  paste0(
    ...
    "events.json?apikey=",
    api.key,
    "&size=200&city=New%20York&countryCode=US&classificationName=Music"
  )
ny.response <- GET(ny.music.url)
ny.body.data <-
  fromJSON(
    content(ny.response, "text"),
    simplifyVector = TRUE,
    simplifyDataFrame = TRUE
  )
...
for (j in c(1:200)) {
  ...
  sea.genre <-
    c(sea.genre, sea.result.data$classifications[[j]]$genre$name)
  sea.city <-
    c(sea.city,
      sea.body.data$`_embedded`$events$`_embedded`$venues[[j]]$city$name)
}
sea.result.data <- sea.result.data %>%
  select(name) %>%
  mutate(
    Genre = sea.genre,
    Minimum = sea.mins[1:200],
    Maximum = sea.max[1:200],
    City = sea.city
  )

sea.result.data <-
  sea.result.data[!duplicated(sea.result.data$name), ] %>%
...
for (j in c(1:200)) {
  ...
  la.genre <-
    c(la.genre, la.result.data$classifications[[j]]$genre$name)
  la.city <-
    c(la.city,
      la.body.data$`_embedded`$events$`_embedded`$venues[[j]]$city$name)
}
...
la.result.data <- la.result.data %>%
  ...
  mutate(
    Genre = la.genre,
    Minimum = la.mins[1:200],
    Maximum = la.max[1:200],
    City = la.city
  )
la.result.data <-
  la.result.data[!duplicated(la.result.data$name),] %>%
  mutate(hover = paste0("Event: ", name, "<br>", "Min: ", Minimum, "<br>", "Max: ", Maximum))
...
for (j in c(1:200)) {
  ...
  ny.genre <-
    c(ny.genre, ny.result.data$classifications[[j]]$genre$name)
  ny.city <-
    c(ny.city,
      ny.body.data$`_embedded`$events$`_embedded`$venues[[j]]$city$name)
}
ny.result.data <- ny.result.data %>%
  select(name) %>%
  mutate(
    Genre = ny.genre,
    Minimum = ny.mins[1:200],
    Maximum = ny.max[1:200],
    City = ny.city
  )

ny.result.data <-
  ny.result.data[!duplicated(ny.result.data$name), ] %>%
  mutate(hover = paste0("Event: ", name, "<br>", "Min: ", Minimum, "<br>", "Max: ", Maximum))

#merged 
sea.la <- merge(sea.result.data, la.result.data, all = T)
sea.la.ny <- merge(sea.la, ny.result.data, all = T)
sea.la.ny <- na.omit(sea.la.ny)
